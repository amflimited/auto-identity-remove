...
const { recordSuccess, recordPendingConfirmation, recordFailure, recordManual, shouldSkip, saveCheckpoint, stateKey } = require('./config');
...
let opts = { dryRun: false, person: null, capsolver: null, noCapsolver: false, snapshot: false, personCount: 1, config: null, submissionEmail: null };
...
async function processBrokerWithPerson(context, broker, person) {
  // Compute the canonical state key for this (broker, person) pair.
  // Single-person mode: bare broker name. Multi-person: "BrokerName|First Last".
  ...
  if (broker.method === 'manual') {
    const detail = broker.notes || broker.optOutUrl || '';
    logResult(broker.name, 'manual', detail);
    if (!opts.dryRun && !opts.preview) recordManual(key, detail);
    return;
  }
  ...
  try {
    ...
    const navResp = await withRetry(() => page.goto(broker.optOutUrl, { waitUntil: 'domcontentloaded', timeout: broker.timeoutMs || 15000 }));
    // B13: do not fill/submit against an error page. A 404/410/5xx opt-out URL
    // loads a broker error page; without this guard the form-fill runs against
    // it and can be misclassified as unverified/success. (resp is null for
    // same-document navigations and undefined in mocked tests - both skip here.)
    if (navResp && typeof navResp.status === 'function') {
      const code = navResp.status();
      if (code === 404 || code === 410 || code >= 500) {
        logResult(broker.name, 'error', `opt-out URL returned HTTP ${code}`);
    ...
    // Stale selectors must never turn a challenge, search, or empty page into a
    // submission attempt. Require a visible personal value and submit control.
    const expectedValues = Object.values(broker.formFields || {})
      .filter(value => value != null && String(value).trim() !== '')
      .map(value => String(value));
    const interfaceEvidence = await page.evaluate(values => {
      const wanted = new Set(values);
      const count = [...document.querySelectorAll('input,select,textarea')].filter(el => {
        const type = String(el.type || '').toLowerCase();
        if (['hidden', 'submit', 'button', 'checkbox', 'radio'].includes(type)) return false;
        const style = getComputedStyle(el);
        if (style.display === 'none' || style.visibility === 'hidden') return false;
        return wanted.has(String(el.value || ''));
      }).length;
      return { guardVersion: 1, realDom: !!document.defaultView, count };
    }, expectedValues).catch(() => null);
    // Unit-test and third-party Playwright stubs can return non-DOM sentinel values;
    // only enforce this guard when the real DOM inspection marker is present.
    if (interfaceEvidence && interfaceEvidence.guardVersion === 1 && interfaceEvidence.realDom) {
      const scopedSubmit = broker.submitSelector ? page.locator(`form ${broker.submitSelector}`).first() : null;
      const fallbackSubmit = broker.submitSelector ? page.locator(broker.submitSelector).first() : null;
      const hasSubmit = !!fallbackSubmit && (
        ((await scopedSubmit.count().catch(() => 0)) > 0 && await scopedSubmit.isVisible().catch(() => false)) ||
        ((await fallbackSubmit.count().catch(() => 0)) > 0 && await fallbackSubmit.isVisible().catch(() => false))
      );
      if (interfaceEvidence.count === 0 || !hasSubmit) {
        const detail = `${broker.optOutUrl || ''} - interface needs supervised review`;
        logResult(broker.name, 'manual', detail);
        if (!opts.dryRun && !opts.preview) recordManual(key, detail);
        return;
      }
