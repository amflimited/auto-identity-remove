 ...
 * @returns {string}
 */
function _buildBodyCCPA(person, authorizedAgentName = '') {
  const agentLines = authorizedAgentName ? [
    `This request is being sent by ${authorizedAgentName} as my authorized service provider.`,
    'I authorized this request using my own information. Please send any identity or',
    'email-confirmation step to the Reply-To address on this message.',
    '',
  ] : [];
  return [
    'To Whom It May Concern,',
    '',
    ...agentLines,
    'I am an Indiana resident requesting the removal of records associated with my',
    'personal information under your privacy policy and any applicable privacy rights.',
    '',
    `Name: ${person.fullName}`,
    `Location: ${person.city}, ${person.state} ${person.zip}`,
    `Email: ${person.email}`,
    `Phone: ${person.phoneFormatted}`,
    '',
    'Please remove or suppress the profiles and records you are able to identify, stop',
    'selling or using them for targeted advertising, and confirm the outcome or any',
    'additional verification you require.',
    '',
    'Thank you,',
    `${person.fullName}`,
  ].join('\n');
 ...
 * @param {number} [personCount]    - total number of persons in the run; used
 *   to derive the correct composite state key (matches broker-runner keying).
 */
async function _sendViaSMTP(broker, person, smtpCfg, submissionEmail, personCount) {
  ...
  // A loopback MTA can legitimately present a locally issued STARTTLS
  // certificate.  This opt-out is explicit in config and must never be inferred
  // for a remote SMTP host.
  if (smtpCfg.tls && typeof smtpCfg.tls === 'object') {
    transportOptions.tls = {
      rejectUnauthorized: smtpCfg.tls.rejectUnauthorized !== false,
    };
  }
  if (smtpCfg.user && smtpCfg.pass) transportOptions.auth = { user: smtpCfg.user, pass: smtpCfg.pass };
  ...
  const body = _pickTemplate(bodyPerson.country)(bodyPerson, smtpCfg.authorizedAgentName || '');
  ...
  if (smtpCfg.replyTo || submissionEmail) mail.replyTo = smtpCfg.replyTo || submissionEmail;

  try {
    await transporter.sendMail(mail);
    loggerMod.logResult(broker.name, 'success', `Email -> ${broker.emailTo}`);
    const key = stateKey(broker.name, person, personCount);
    configMod.recordSuccess(key, `email to ${broker.emailTo}`);
  } catch (err) {
...
async function sendOptOutEmails(brokers, cfg, _platform, opts = {}) {
  ...
  for (const broker of emailBrokers) {
    ...
    for (const person of persons) {
      ...
      if (smtpCfg) {
        await _sendViaSMTP(broker, person, smtpCfg, submissionEmail, personCount);
      } else {
        loggerMod.logResult(
          broker.name,
          'manual',
