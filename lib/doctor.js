...
const { isPlaceholderKey } = require('./captcha');
...
async function checkCapsolver(cfg, opts = {}) {
  ...
  if (isPlaceholderKey(apiKey)) {
    return { ok: true, hint: 'skipped - placeholder key detected; set your real 2Captcha key' };
  }

  const headRequest = opts.headRequest || defaultHeadRequest;
  const base = (process.env.CAPTCHA_API_BASE || 'https://api.2captcha.com').replace(/\/+$/, '');
  const host = base.replace(/^https?:\/\//, '');

  try {
    await headRequest(`${base}/`);
    return { ok: true, hint: `${host} reachable` };
  } catch (e) {
    return { ok: false, hint: `${host} unreachable - ${e.message}` };
  }
}
...
async function runDoctor(opts = {}) {
  ...
  // Overlay env-provided secrets (e.g. TWOCAPTCHA_API_KEY) so the solver check
  // reflects the same key the real run uses, not just what is in config.json.
