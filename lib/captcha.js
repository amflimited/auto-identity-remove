...
// ---------------------------------------------------------------------------
// Solver provider. This fork uses 2Captcha, whose JSON API
// (api.2captcha.com/createTask + /getTaskResult) is Anti-Captcha/CapSolver
// compatible: same { clientKey, task } request and { status, solution } reply,
// with slightly different task-type strings (mapped below) and errors surfaced
// via a non-zero errorId instead of status:'failed'. Override the base with
// CAPTCHA_API_BASE to point back at CapSolver or another compatible provider.
const SOLVER_BASE = (process.env.CAPTCHA_API_BASE || 'https://api.2captcha.com').replace(/\/+$/, '');

// CapSolver task-type name -> 2Captcha task-type name.
const TASK_TYPE = {
  ...
  AntiTurnstileTaskProxyLess: 'TurnstileTaskProxyless',
  AntiAwsWafTask:             'AntiAwsWafTask', // not offered by 2Captcha; will error out and log captcha_failed
};

/**
 * Create a solver task on the configured provider, remapping the task type to
 * the provider's naming. Returns the raw axios response ({ data: { taskId } }).
 * @param {object} axios injected axios-compatible instance
 * @param {string} clientKey solver API key
 * @param {object} task CapSolver-shaped task object (type + params)
 */
function createTask(axios, clientKey, task) {
  const type = TASK_TYPE[task.type] || task.type;
  return axios.post(`${SOLVER_BASE}/createTask`, { clientKey, task: { ...task, type } });
}
...
async function pollCapSolver(taskId, opts) {
  ...
  for (let i = 0; i < maxTries; i++) {
    ...
    const { data } = await axios.post(`${SOLVER_BASE}/getTaskResult`, { clientKey, taskId });
    // 2Captcha surfaces failures via a non-zero errorId (no status:'failed').
    if (data.errorId) return null;
...
async function solveRecaptcha(page, capsolver) {
  ...
  if (!key || isPlaceholderKey(key)) {
    console.log('     info  No captcha-solver key - add one to config.json to auto-solve CAPTCHAs');
    return false;
  }
  try {
    ...
    const { data: createData } = await createTask(axios, key, {
      type: 'ReCaptchaV2TaskProxyless', websiteURL: pageUrl, websiteKey: siteKey,
    });
    ...
    // 2Captcha may return the token as gRecaptchaResponse or token; accept either.
    await page.evaluate(findRecaptchaCallback(solution.gRecaptchaResponse || solution.token || ''));
    console.log('     reCAPTCHA solved');
    return true;
  } catch (err) {
    console.log(`     2Captcha reCAPTCHA error: ${err.message.slice(0, 60)}`);
    return false;
  }
}
...
async function solveHcaptcha(page, capsolver) {
  const key = capsolver?.apiKey;
  if (!key || isPlaceholderKey(key)) {
    console.log('     info  No captcha-solver key - cannot solve hCAPTCHA automatically');
    return false;
  }
  try {
    const axios   = require('axios').create({ timeout: 30000 });
    ...
    console.log('     Solving hCAPTCHA via 2Captcha...');
    const { data: createData } = await createTask(axios, key, {
    ...
    const solution = await pollCapSolver(taskId, { clientKey: key, axios });
    if (!solution) return false;
    // 2Captcha returns the hCaptcha token in solution.token (CapSolver used
    // gRecaptchaResponse); accept either so the same code works on both.
    ...
    console.log(`     2Captcha hCAPTCHA error: ${err.message.slice(0, 60)}`);
    return false;
  }
}
...
async function solveTurnstile(page, capsolver) {
  const key = capsolver?.apiKey;
  if (!key || isPlaceholderKey(key)) {
    console.log('     info  No captcha-solver key - cannot solve Turnstile automatically');
    return false;
  }
  try {
    const axios   = require('axios').create({ timeout: 30000 });
    ...
    console.log('     Solving Cloudflare Turnstile via 2Captcha...');
    const { data: createData } = await createTask(axios, key, {
      type: 'AntiTurnstileTaskProxyLess', websiteURL: pageUrl, websiteKey: siteKey,
    });
    const taskId = createData.taskId;
    if (!taskId) return false;
    ...
    console.log(`     2Captcha Turnstile error: ${err.message.slice(0, 60)}`);
    return false;
  }
}
...
async function solveRecaptchaV3(page, capsolver, broker) {
  const key = capsolver?.apiKey;
  if (!key || isPlaceholderKey(key)) {
    console.log('     info  No captcha-solver key - cannot solve reCAPTCHA v3 automatically');
    return false;
  }
  try {
    const axios   = require('axios').create({ timeout: 30000 });
    ...
    console.log(`     2Captcha reCAPTCHA v3 error: ${err.message.slice(0, 60)}`);
    return false;
  }
}
...
async function solveAwsWaf(page, capsolver) {
  const key = capsolver?.apiKey;
  if (!key || isPlaceholderKey(key)) {
    console.log('     info  No captcha-solver key - cannot solve AWS WAF CAPTCHA automatically');
    return false;
  }
  try {
    const axios   = require('axios').create({ timeout: 30000 });
    ...
    console.log('     Solving AWS WAF CAPTCHA via 2Captcha...');
    const { data: createData } = await createTask(axios, key, {
      type: 'AntiAwsWafTask',
      websiteURL: pageUrl,
    ...
    console.log(`     2Captcha AWS WAF error: ${err.message.slice(0, 60)}`);
    return false;
  }
}
...
module.exports = {
  // Provider (2Captcha) plumbing - exported for testing
  SOLVER_BASE,
  TASK_TYPE,
  createTask,
  // Shared helpers
